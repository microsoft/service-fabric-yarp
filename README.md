...
* [Using SF YarpProxy Application](#using-sf-yarpproxy-application)
  ...
  * [Tracing](#tracing)
  * [Querying Fabric Discovery Service](#querying-fabric-discovery-service)
...
>[!NOTE]
> At this time, this is a reverse proxy built-in replacement and not a generic service fabric 'gateway' able to handle partition queries, but that might be added (via customer written plugins or similar) in the future.
...
>[!NOTE]
> If a stateful service only has 1 partition then do not include PartitionID as a query parameter in the request.
...
## Querying Fabric Discovery Service

Fabric Discovery Service is a stateless service that is responsible for discovering Service Fabric services that are configured to use YARP via Service Manifest Extensions. It is also responsible for periodically refreshing the YARP configuration from the Service Fabric cluster. The configuration is stored in a JSON format and can be queried via the Fabric Discovery Service REST API. 

Determine which node and port the Fabric Discovery Service is running on by using the Service Fabric Explorer or the Get-ServiceFabricNode PowerShell command. Then query the Fabric Discovery Service REST API using the following format:

```text
http(s)://<Cluster FQDN | internal IP>:<port>/api/v1/yarpconfig
```

example powershell query:
  
```powershell
$yarpconfig = irm http://localhost:30016/api/v1/yarpconfig
$yarpconfig | ConvertTo-Json -Depth 10
```
...
```json
{
  "clusters": [
    {
      "clusterId": "fabric:/sf-sample-weatherforecast/VotingWebCore",
      ...
      "httpClient": {
        "sslProtocols": null,
        "dangerousAcceptAnyServerCertificate": null,
        ...
        "enableMultipleHttp2Connections": null,
        "requestHeaderEncoding": null,
        "responseHeaderEncoding": null
      },
      "httpRequest": {
      ...
      "destinations": {
        "37f9e9e8-894a-4f9f-8d6a-2986911e679f/133498300363644139": {
          "address": "http://work:8081/",
          "health": null,
          "metadata": {
            "__SF.PartitionId": "37f9e9e8-894a-4f9f-8d6a-2986911e679f",
            "__SF.ReplicaId": "133498300363644139"
          },
          "host": null
        }
      },
      "metadata": {
        "__SF.ApplicationTypeName": "sf-sample-weatherforecastType",
        "__SF.ApplicationName": "fabric:/sf-sample-weatherforecast",
        "__SF.ServiceTypeName": "VotingWebCoreType",
        "__SF.ServiceName": "fabric:/sf-sample-weatherforecast/VotingWebCore"
      }
    }
  ],
  "routes": [
    {
      "routeId": "fabric%3A%2Fsf-sample-weatherforecast%2FVotingWebCore:defaultRoute",
      "match": {
        "methods": null,
        "hosts": null,
        "path": "/sf-sample-weatherforecast/VotingWebCore/{**catchall}",
      ...
      "transforms": [
        {
          "PathRemovePrefix": "/sf-sample-weatherforecast/VotingWebCore"
        }
      ]
    }
  ],
  "asOf": "2024-01-15T17:09:55.7746886-05:00"
}
```
